...
void ExecuteAndMessage(char * sql,CEditArea* editArea,CHustBaseDoc *pDoc){//根据执行的语句类型在界面上显示执行结果。此函数需修改
	...
	RC rc = execute(sql,pDoc);//非查询语句则执行其他SQL语句，成功返回SUCCESS
	int row_num = 0;
	char**messages;
	switch(rc){
	case SUCCESS:
...
RC Insert(char *relName,int nValues,Value *values){
	...
	char *value=(char *)malloc(200);//读取数据表信息
	memset(value,0,200);
	RID *rid;
