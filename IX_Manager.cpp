...
RC OpenIndexScan(IX_IndexScan *indexScan,IX_IndexHandle *indexHandle,CompOp compOp,char *value)
{
	...
	//鍒濆?嬪寲椤甸潰鍙枫�佺储寮曢」缂栧彿銆侀〉闈㈠彞鏌?
	switch (compOp) //灏忎簬銆佸皬浜庣瓑浜庝笌绛変簬浠庢渶灏忕殑绱㈠紩椤瑰紑濮嬫煡鎵?
	{
	case NO_OP:
	case LEqual:
	case LessT:
	...
	int startPageNumber=FindNode(indexHandle,value);  //鎵惧埌鎼滅储寮�濮嬬殑绱㈠紩鍊兼墍鍦ㄨ妭鐐?
	GetThisPage(&indexHandle->fileHandle,startPageNumber,indexScan->pfPageHandle);
	IX_Node *startPageControl=(IX_Node *)(indexScan->pfPageHandle->pFrame->page.pData+sizeof(IX_FileHeader));  //鑾峰緱寮�濮嬮〉鐨勭储寮曡?板綍淇℃伅
	int indexOffset,rtn;
...
{
	int pageInsertNumber=FindNode(indexHandle,pData); //鏍规嵁杈撳叆鐨勬暟鎹?鎵惧埌鍗冲皢鎿嶄綔鐨勮妭鐐?
...
void RecursionInsert(IX_IndexHandle *indexHandle, void *pData, const RID *rid, PF_PageHandle *pageInsert)
{
	...
	{
		if (0 == posInsert)															// 新插入的值是页面第一个节点,则调整父页面的值
		{
			PageNum nodePageNum;
			PF_PageHandle *parentPageHandle = new PF_PageHandle;

			GetPageNum(pageInsert, &nodePageNum);									//本页面的页号
			GetThisPage(&indexHandle->fileHandle, pageControl->parent, parentPageHandle);			//本页面的父亲
		...
		// else 新插入的值部是页面第一个节点, 直接退出
	}
	else
	{
		//绱㈠紩椤规暟杈惧埌浜嗘渶澶э紝鑺傜偣鍒嗚??
		int splitOffset = int(pageControl->keynum/2+1);          //鑺傜偣绱㈠紩璁板綍鍙栧崐锛屽悜涓婂彇鏁?
		PF_PageHandle *brotherNode = new PF_PageHandle;                              //涓哄綋鍓嶈妭鐐瑰垎閰嶄竴涓?鍏勫紵鑺傜偣
		AllocatePage(&indexHandle->fileHandle, brotherNode);
		pageControl->brother = brotherNode->pFrame->page.pageNum;  //鏍囪?板厔寮熻妭鐐圭殑椤靛彿
		IX_Node *broPageControl = (IX_Node *)(brotherNode->pFrame->page.pData + sizeof(IX_FileHeader)); //鍏勫紵鑺傜偣鐨勬帶鍒朵俊鎭?
		broPageControl->keys = brotherNode->pFrame->page.pData + sizeof(IX_FileHeader)+sizeof(IX_Node); //璁＄畻鍏勫紵鑺傜偣鐨勭储寮曞尯涓庢暟鎹?鍖?
		...
		memcpy(broPageControl->keys, pageControl->keys + splitOffset, broPageControl->keynum * indexHandle->fileHeader.keyLength); //搬移索引区数据
		pageControl->keynum = splitOffset;         //设置当前节点的实际索引数
		memcpy(broPageControl->rids, pageControl->rids + splitOffset, broPageControl->keynum * sizeof(RID)); //搬移指针区数据
		broPageControl->is_leaf = pageControl->is_leaf;  //鏍囪?板厔寮熻妭鐐瑰彾瀛愯妭鐐瑰睘鎬?
		broPageControl->brother = -1;    //鍏勫紵鑺傜偣鏆傛椂娌℃湁鍙冲厔寮熻妭鐐?
		MarkDirty(brotherNode);
		UnpinPage(brotherNode);
		free(brotherNode);

		//妫�鏌ユ槸鍚︽槸褰撳墠鐨勬牴鑺傜偣鍦ㄥ垎瑁傦紙鏄?鍚︽湁鐖剁粨鐐癸級
		if(pageControl->parent == 0)    //褰撳墠鑺傜偣鏄?鏍硅妭鐐?
		{
			PF_PageHandle *parentNode = new PF_PageHandle;
			AllocatePage(&indexHandle->fileHandle, parentNode);
			IX_Node *parentPageControl = (IX_Node *)(parentNode->pFrame->page.pData + sizeof(IX_FileHeader));;  //鐖剁粨鐐规帶鍒朵俊鎭?
			//鍒濆?嬪寲鐖剁粨鐐逛俊鎭?
			parentPageControl->keynum = 2; //鍙?鏈変袱涓?绱㈠紩椤圭洰
			...
			indexHandle->fileHeader.rootPage = parentNode->pFrame->page.pageNum;  //璁剧疆褰撳墠鐨勬牴鑺傜偣浣嶇疆
			memcpy(parentPageControl->keys, pageControl->keys, indexHandle->fileHeader.keyLength);  //褰撳墠鑺傜偣鐨勭??涓�涓?绱㈠紩鍊?
			...
			parentPageControl->rids->bValid = true;
			parentPageControl->rids->pageNum = brotherNode->pFrame->page.pageNum;  //涓�涓?鎸囬拡鎸囧悜鍏勫紵鑺傜偣
			parentPageControl->rids->slotNum = 0;   //鍐呰妭鐐圭殑鎸囬拡鐨勬Ы鍊奸兘涓?0
			pageControl->parent = parentNode->pFrame->page.pageNum;  //褰撳墠鑺傜偣鎸囧悜鐖剁粨鐐?
			broPageControl->parent = parentNode->pFrame->page.pageNum;  //鍏勫紵鑺傜偣鎸囧悜鐖剁粨鐐?
		...
		else
		{
			...
			if(posInsert != 0)  //鍓嶉潰鎻掑叆鐨勬椂鍊欐彃鍦ㄤ簡褰撳墠鑺傜偣鐨勬渶宸︿晶锛岄渶瑕佹洿鏂扮埗鑺傜偣鐨勭储寮曞�?
 ...
 */
	...
	pageNum pageNumber = FindNode(indexHandle, pData); //根据输入的数据找到即将操作的节点
	PF_PageHandle *pageDelete = new PF_PageHandle;
	GetThisPage(indexHandle->fileHandle, pageNumber, pageDelete);
	//调用递归函数
...
RC RecursionDelete(IX_IndexHandle *indexHandle, void *pData, const RID *rid, PF_PageHandle *pageDelete)
{
	...
	if(pageControl->keynum >= threshold)							// 索引项数符合规定,没有下溢
	{
		if(offset == 0)	// 删除的是页面的第一个结点,需调整父页面的值
		{
			...
			GetPageNum(pageDelete, &nodePageNum);									//本页面的页号
...
void deleteOrAlterParentNode(PF_PageHandle *parentPageHandle, PF_FileHandle *fileHandle, int order, AttrType attrType, int attrLength, PageNum nodePageNum, void *pData, int parentOrder, bool isDelete)
{
	...
	while(true) 
	{
		...
		else
		{
			...
			{
				MarkDirty(parentPageHandle);		// 标记为脏页
...
RC CreateIndex(const char * fileName,AttrType attrType,int attrLength){
	...
	// 椤甸潰涓婃坊鍔?<绱㈠紩鎺у埗淇℃伅>锛屽叾涓璻ootPage鍜宖irst_leaf榛樿?よ?句负1椤碉紝鏈夎??鍚庢湡鏀?
...
RC CloseIndex(IX_IndexHandle *indexHandle){
	...
	if(!indexHandle->bOpen)
		return IX_ISCLOSED;
	if(CloseFile(&indexHandle->fileHandle))	// 鐢╢ilename鍏抽棴鏂囦欢? 鍏抽棴鏂囦欢娌℃湁瀵瑰簲鐨勬暟鎹?缁撴瀯
		return FAIL;
